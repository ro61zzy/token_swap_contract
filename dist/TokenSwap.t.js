...
describe("TokenSwap Contract with GUZ and W3B", function () {
    ...
    beforeEach(async function () {
        ...
        guzToken = await Token.deploy("GUZ Token", "GUZ", ethers.utils.parseEther("1000"));
        w3bToken = await Token.deploy("W3B Token", "W3B", ethers.utils.parseEther("1000"));
    ...
    describe("Create Order with GUZ and W3B", function () {
        it("create an order and transfer GUZ to the contract", async function () {
            ...
            await expect(tokenSwap
                .connect(addr1)
                .createOrder(guzToken.address, ethers.utils.parseEther("100"), w3bToken.address, ethers.utils.parseEther("20"))).to.emit(tokenSwap, "OrderCreated");
            expect(await guzToken.balanceOf(tokenSwap.address)).to.equal(ethers.utils.parseEther("100"));
    ...
    describe("Fulfill Order with GUZ and W3B", function () {
        it("should fulfill an order and transfer GUZ and W3B between parties", async function () {
            ...
            await tokenSwap
                .connect(addr1)
                .createOrder(guzToken.address, ethers.utils.parseEther("100"), w3bToken.address, ethers.utils.parseEther("20"));
            await w3bToken
                .connect(addr2)
                .approve(tokenSwap.address, ethers.utils.parseEther("20"));
            await expect(tokenSwap.connect(addr2).fulfillOrder(0)).to.emit(tokenSwap, "OrderFulfilled");
            expect(await guzToken.balanceOf(await addr2.getAddress())).to.equal(ethers.utils.parseEther("100"));
            expect(await w3bToken.balanceOf(await addr1.getAddress())).to.equal(ethers.utils.parseEther("20"));
            const order = await tokenSwap.orders(0);
        ...
        it("should not fulfill an inactive order", async function () {
            ...
            await expect(tokenSwap.connect(addr2).fulfillOrder(0)).to.be.revertedWith("Order is not active");
        });
    });
    describe("Cancel Order with GUZ and W3B", function () {
        it("should allow the creator to cancel the order and reclaim GUZ tokens", async function () {
            ...
            await tokenSwap
                .connect(addr1)
                .createOrder(guzToken.address, ethers.utils.parseEther("100"), w3bToken.address, ethers.utils.parseEther("20"));
            await expect(tokenSwap.connect(addr1).cancelOrder(0)).to.emit(tokenSwap, "OrderCancelled");
            expect(await guzToken.balanceOf(tokenSwap.address)).to.equal(0);
            expect(await guzToken.balanceOf(await addr1.getAddress())).to.equal(ethers.utils.parseEther("100"));
            const order = await tokenSwap.orders(0);
            expect(order.active).to.be.false;
        });
        it("should not allow a non-creator to cancel the order", async function () {
            ...
            await tokenSwap
                .connect(addr1)
                .createOrder(guzToken.address, ethers.utils.parseEther("100"), w3bToken.address, ethers.utils.parseEther("20"));
            await expect(tokenSwap.connect(addr2).cancelOrder(0)).to.be.revertedWith("Only creator can cancel the order");
        });
    });
});
